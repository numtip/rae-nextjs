...
export async function GET() {
  try {
    ...
    const findResult = await sandbox.runCommand(`find . -name node_modules -prune -o -name .git -prune -o -name dist -prune -o -name build -prune -o -type f ( -name *.jsx -o -name *.js -o -name *.tsx -o -name *.ts -o -name *.css -o -name *.json ) -print`);
    ...
    for (const filePath of fileList) {
      try {
        ...
        const statResult = await sandbox.runCommand(`stat -c %s ${filePath}`);
